...
1. Простая в использовании фабрика `vkstream` — позволяет создать поток с желаемым набором [правил](https://vk.com/dev/streaming_api_docs?f=2.%20%D0%A4%D0%BE%D1%80%D0%BC%D0%B0%D1%82%20%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB) и считывать из него данные;
2. Низкоуровневые `VKStreamingAPI` и `VKWebSocket` — для случаев, когда необходимо реализовать менее тривиальную логику, чем сбор сообщений по предзаданным правилам.
...
`VKWebSocket` — класс для чтения потока через websocket подключение к VK Streaming API. Так же как и объект создаваемый фабрикой `vkstream`, инстанс `VKWebSocket` имплементирует Readable Stream, а значит чтение потока может осуществляться как через обработку события `data`, так и с использованием `pipe`.

Для инстанцирования `VKWebSocket` необходимо указать адрес для установления соединения и опциональный объект с параметрами подключения.
